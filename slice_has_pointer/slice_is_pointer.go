...
func changePerson(slice []Person) error {
	...
	for _, person := range slice {
		// personInner := person
		sliceRes = append(sliceRes, &person)

		person.Name = "1111111"
		person.Age = 100
	}

	// for _, sliceR := range sliceRes {
	// 	sliceR.Name = "hellow 123456"
	// 	sliceR.Age = 123456
	// }

	log.Printf("changePerson, sliceRes: %+v", *sliceRes[0])
	log.Printf("changePerson, sliceRes: %+v", *sliceRes[1])
...
func changePersonPointer(slice []*Person) error {
	...
	log.Printf("changePersonPointer, sliceRes: %+v", *sliceRes[0])
	log.Printf("changePersonPointer, sliceRes: %+v", *sliceRes[1])
